...
static int 		get_str_len(long long nbr, char *base)
{
...
static void 	convert_nbr(long long nbr, char *base, char *str, int index)
{
	...
	if (nbr < 0)
	{
		...
		value = (unsigned long long)nbr;
	if (value > len_base - 1)
		convert_nbr((long long)(value / len_base), base, str, index - 1);
	str[index] = base[value % len_base];
}

char 			*ft_lltoa_base_str(int nbr, char *base)
{
	int		str_len;
	char	*str;
