...
int				print_d(va_list ap, t_buf *buf, t_conv *conv)
{
	long long int	nb;
	unsigned int	len;
	int				prefix_len;
...
void			print_s_prefix(long long nb, t_conv *conv, t_buf *buf)
{
	if (nb >= 0 && conv->plus)
		putc_no_format(buf, '+');
